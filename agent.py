...
class Agent:
    ...
    TRADING_TAX = 0 # 거래 수수료 미고려 (실제 0.3%)
    ...
    def get_states(self):
        ...
        self.ratio_portfolio_value = self.prtfolio_value / self.initial_balance
        return (
            self.ratio_hold,
    ...
    def act(self, action, confidence):
        ...
        # 즉시 보상 판단
        self.immediate_reward = 1 if porfitloss >= 0 else -1

        # 지연 보상 판단
        if profitloss > self.delayed_reward_threshold:
            delayed_reward = 1
            # 목표 달성률을 달성하여 기준 포트폴리오 가치 갱신
